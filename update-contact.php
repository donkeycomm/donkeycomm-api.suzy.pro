...
// Function to update contact in the database
function updateContactInDatabase($id, $fields) {
    ...
    $sql = "UPDATE contacts SET $set_clause WHERE id = ?";
    ...
    try {
        ...
        error_log("Error in updateContactInDatabase: " . $e->getMessage());
        return false;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array(0, $rolesArray) || in_array(1, $rolesArray)) {
        ...
        $fields['firstname'] = isset($_POST['firstname']) ? $_POST['firstname'] : null;
        $fields['lastname'] = isset($_POST['lastname']) ? $_POST['lastname'] : null;
        $fields['email'] = isset($_POST['email']) ? $_POST['email'] : null;
        $fields['phone'] = isset($_POST['phone']) ? $_POST['phone'] : null;
        $fields['company'] = isset($_POST['company']) ? $_POST['company'] : null;
        $fields['title'] = isset($_POST['title']) ? $_POST['title'] : null;
        $fields['contact_list'] = isset($_POST['contact_list']) ? json_encode(json_decode($_POST['contact_list'], true)) : null;
        $fields['description'] = isset($_POST['description']) ? $_POST['description'] : null;

        if (updateContactInDatabase($_POST['id'], $fields)) {
            $successMessages[] = 'Contact updated successfully';
        } else {
            $errorMessages[] = 'Failed to update contact in database';
        }
    } else {
        $errorMessages[] = 'You\'re not authorized to update files';
    }
