...
function getUserRolesFromDatabase($user_email, $jwt) {
    ...
    try {
        ...
        if ($user && isset($user['access_levels'])) {
    ...
    //check if json field user_roles contains 0 or 1
    ...
    if(in_array(0, $rolesArray) || in_array(1, $rolesArray)){
        try {
            ...
            $sql = "SELECT * FROM `access_levels` WHERE `access_level` != 0 AND `access_level` != 1 ORDER BY `access_level` ASC";
